    ...
    methods:{
        ...
        generateInvoice(){
            this.form.post('api/invoices').then((data)=>{
                // this.info = data;
                // id
                this.tenant_id = data.data.tenant_id;
                window.open('/reports/invoice/'+this.tenant_id,'popUpWindow','height='+screen.height+',width='+screen.width+',left=100,top=100,resizable=no,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no, status=yes');            
            })
        }
